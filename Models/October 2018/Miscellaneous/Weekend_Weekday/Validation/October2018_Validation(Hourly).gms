...
$ontext
Title Optimization model for Glen Canyon Dam releases to favor Bugs population (October 2018)
...
initstorage                  Initial reservoir storage on 1st October 2018 (acre-ft)/11016261/
...
observed_release(d,p)        observed release during October 2018 averaged over hourly timestep (cfs)
...
**The observed hydrograph has sometimes when the release is below 8000 cfs. Hence, for this validation we will ignore minimum release constraint.
...
*===================================================
$CALL GDXXRW.EXE input=Input_October2018.xlsx  output= Valid_October2018(Hourly).gdx  par=observed_release rng=Observed_Release(Hr)!A1:Y32 par=EnergyRate rng=Rates!A1 Rdim=1  set=d rng=day!A1 Rdim=1  par=Inflow rng=inflow!A1 Rdim=1 par=evap rng=Evaporation!A1 Rdim=1

*Write the input Data into a GDX file
$GDXIN  Valid_October2018(Hourly).gdx
...
Display d,inflow, p, EnergyRate,observed_release,evap;
*===============================================
SCALAR
...
EQ1__ResMassBal(d)..         storage(d) =e= initstorage$(ord(d)eq 1)+ storage(d-1)$(ord(d)gt 1)+ (inflow(d)*Convert*24)- sum(p,ReleaseVol(d,p))-evap(d);
...
Execute_Unload " Valid_October2018(Hourly).gdx";
* Dump the gdx file to an Excel workbook
Execute "gdx2xls  Valid_October2018(Hourly).gdx"
