...
Use_Sim(d,p) $((ord(d)>= 1  and ord(d)<= 3)
                    OR(ord(d)>= 8 and ord(d)<= 10)
                   OR (ord(d)>=15 and ord(d)<=17)
                  OR (ord(d)>=25 and ord(d)<=24)
                  OR (ord(d)>=29 and ord(d)<=30)) = 1;
...
Execute_Unload "Junel8_3bugflow_dailyCV.gdx";
* Dump the gdx file to an Excel workbook
Execute "gdx2xls Junel8_3bugflow_dailyCV.gdx"
