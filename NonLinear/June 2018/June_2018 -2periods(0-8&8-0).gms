...
;
* Here weekdays are priced differet then the weekends (days used in the use sim parameter)
Table EnergyRate(d,p) "Energy prices during different days and periods ($ MWh)"
      pLow     pHigh
d1    24.56    62.21
d2    24.56    24.56
d3    24.56    24.56
...
*EQ6_Energyrate(d,p)..        Energyrate_vari(d,p)=e= EnergyRate(d,p);
*Equation 6 is just making the energy rate same for all days. Here, we have energy rates varies by day of the week. Please uncomment the equation if you want energy rate to remain same throught week. And get ride of energy rate table too.
...
EQ13_EnergyRevenue(f)$(ord(f) eq 2)..         ObjectiveVal(f)=e= Obj_dir(f)* sum((d,p),Energy_Gen(d,p)*EnergyRate(d,p));
...
Execute_Unload "Junel8_2phydobj_0to8.gdx";
* Dump the gdx file to an Excel workbook
*Execute "gdx2xls Junel8_2phydobj_0to8.gdx"
Execute "GDXXRW Junel8_2phydobj_0to8.gdx"
