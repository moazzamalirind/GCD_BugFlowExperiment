...
Scenario 2 where all the constraints are kept..
...
XStore(FlowType,Days)            Store Energy Generated during different types of days over different cases (MWh)
RStore(FlowType,Days,p)          Store Release values during different types of days over different cases (cfs)
...
Weekday_Releases(p)              observed releases (cfs) during weekdays of June 2018/pLow  11363.14,pHigh  15738.38/
Sunday_offPeak                   observed releases (cfs) during weekends of June 2018 /10433.23/
...
Table Days_Distribution(FlowPattern,d) "number of saturdays sundays and weekdays in a month"
...
EQ15_Steady_Weekdays(FlowPattern,d,p)            Constraining the steady weekdays flows equal to steady sundays flows minus the offset(cfs)


EQ16_OnPeak_UnsteadyDay(FlowPattern,d)           Constraining on-peak unsteady day release to be greater than off-peak unsteady release on the same day (cfs)

EQ17_EnergyGen_Max(FlowPattern,d,p)              Maximum Energy Generation Limit of the Glen Caynon Dam(MW)
EQ18_EnergyGen(FlowPattern,d,p)                  Energy generated in each period p during different day types (MWh)
*Objective Functions
...
EQ20_Observed_weekday(p)                         Observed weekdays releases during June 2018
EQ21_Observed_Sundayoff                          Observed release during off peak sundays June 2018
;
...
EQ4_MaxR(FlowPattern,d,p)$(Days_Distribution(FlowPattern,d) gt 0)..                     Release(FlowPattern,d,p)=l= maxRel;

EQ5_MinR(FlowPattern,d,p)$(Days_Distribution(FlowPattern,d) gt 0)..                     Release(FlowPattern,d,p)=g= minRel;

EQ6_ZeroDays(FlowPattern,d,p)$(Days_Distribution(FlowPattern,d)eq 0)..                  Release(FlowPattern,d,p)=e=0;

EQ7_Rel_Range(FlowPattern,d)$(Days_Distribution(FlowPattern,d) gt 0)..                  Release(FlowPattern,d,"pHigh")- Release(FlowPattern,d,"pLow")=l=Daily_RelRange;


EQ9_RelVolume..                                                                Released_vol=e= sum(FlowPattern,sum(d, sum(p, Release(FlowPattern,d,p)*Convert*Duration(p))*Days_Distribution(FlowPattern,d)));

*Managerial Constraints

EQ10_SteadyFlow_Day(FlowPattern,d)$(Days_Distribution("Steady",d) gt 0)..               Release("Steady",d,"pHigh") =e= Release("Steady",d,"pLow");
...
EQ14_Steady_Saturdays(FlowPattern,d,p)$(Days_Distribution("Steady","Saturday")gt 0)..   Release("Steady","Saturday",p)=e= Release("Steady","Sunday",p);

EQ15_Steady_Weekdays(FlowPattern,d,p)$(Days_Distribution("Steady","Weekday")gt 0)..     Release("Steady","Weekday",p)=e= Release("Steady","Sunday",p) ;

EQ16_OnPeak_UnsteadyDay(FlowPattern,d)$(Days_Distribution("Unsteady",d)gt 0)..          Release("Unsteady",d,"pHigh")=g= Release("Unsteady",d,"pLow");
...
EQ19_EnergyRevenue..                                                           ObjectiveVal=e= sum(FlowPattern,sum(d, sum(p,Energy_Gen(FlowPattern,d,p)*Energy_Rate(d,p))*Days_Distribution(FlowPattern,d)));

EQ20_Observed_weekday(p)..                                                     Release("Unsteady","Weekday",p)=e=Weekday_Releases(p) ;

EQ21_Observed_Sundayoff..                                                      Release("Steady","Sunday","pLow")=e= Sunday_offPeak;
...
* XStore store the energy generated (Mwh/day) during different types of days
   XStore(FlowType,Days)= sum(p,Energy_Gen.L(FlowType,Days,p))+ EPS;

* RStore store the reservoir releases (cfs) during different types of days and scenarios.
   RStore(FlowType,Days,p)= Release.L(FlowType,Days,p)+ EPS;
);
...
Execute_Unload "Valid_SatModel_S2.gdx";
* Dump the gdx file to an Excel workbook
Execute "gdx2xls Valid_SatModel_S2.gdx"
