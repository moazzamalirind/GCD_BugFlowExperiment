...
Use_Sim(d,p) $((ord(d)>= 1  and ord(d)<= 3)
                    OR(ord(d)>= 7 and ord(d)<= 10)
                   OR (ord(d)>=14 and ord(d)<=17)
                  OR (ord(d)>=21 and ord(d)<=24)
                  OR (ord(d)>=28 and ord(d)<=30)) = 1;

*Setting flow for selected days to 10000 cfs.
...
Execute_Unload "Junel8_4bugflow_dailyCV.gdx";
* Dump the gdx file to an Excel workbook
Execute "gdx2xls Junel8_4bugflow_dailyCV.gdx"
