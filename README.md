...
2. You are given two code files (March2018_Validation(Hourly).gms and March2018_validation(2periods).gms) in the validation folder. You are required to run both codes individually. a) Hourly model (hourly releases and energy prices are model inputs) b) Saturday-Sunday-Weekday model (periodic releases and energy prices for different daytypes are model inputs). To the run the code, you require input file (e.g., Input_March_2018.xlsx) to be available in the same project folder.
...
4.   Note: Details of the hourly releases and energy prices, as well as, estimation of periodic releases and energy prices are present in excel file " Input_August2018.xlsx".
Now open GAMS IDE. Click start => gamside or if you have IDE icon on desktop double click that. A blank GAMS interface will show up.
