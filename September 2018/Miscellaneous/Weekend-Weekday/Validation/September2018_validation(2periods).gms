...
release(p)                       Average release observed during each period (p) over the month (cfs) /pLow 10028.41, pHigh 12811.5/
...
Steady_Release                   Steady release value of the hydrograph (cfs) /11058.34/
...
EnergyRate(p)                Energy prices on weekdays ($ per MWH) /pLow 52.19, pHigh 70.006/
weekendRate(p)               Energy prices on weekends ($ per MWH) /pLow 52.19, pHigh 52.19/
Duration(p)                  Duration of period (hours)
Steady_Days                  The number of steady low flow days (weekend during September 2018)
;
...
*===================================================
$CALL GDXXRW.EXE input=Input_September2018.xlsx output=Valid_September2018(2Periods).gdx set=d rng=day!A1 Rdim=1 par=Inflow rng=inflow!A1 Rdim=1  par=evap rng=Evaporation!A1 Rdim=1

*Write the input Data into a GDX file
$GDXIN Valid_September2018(2Periods).gdx
...
weekdays                      Number of Weekdays in month/20/
...
Execute_Unload "Valid_September2018(2Periods).gdx";
* Dump the gdx file to an Excel workbook
Execute "gdx2xls Valid_September2018(2Periods).gdx"
