...
release(p)                       Average release observed during each period (p) over the month (cfs) /pLow 11081.622, pHigh 13694.02/
...
Steady_Release                   Steady release value of the hydrograph (cfs) /11384.76/
**Steady release is average of all the releases during weekends.

initstorage                  Initial reservoir storage on 1st April 2018 (acre-ft)/12948951/
...
EnergyRate(p)                Energy prices on weekdays ($ per MWH) /pLow 38.24, pHigh 55.054/
weekendRate(p)               Energy prices on weekends ($ per MWH) /pLow 38.24, pHigh 38.24/
Duration(p)                  Duration of period (hours)
Steady_Days                  The number of steady low flow days (weekend during April 2018)
;
...
*===================================================
$CALL GDXXRW.EXE input=Input_April2018.xlsx output=Valid_April2018(2Periods).gdx set=d rng=day!A1 Rdim=1 par=Inflow rng=inflow!A1 Rdim=1  par=evap rng=Evaporation!A1 Rdim=1

*Write the input Data into a GDX file
$GDXIN Valid_April2018(2Periods).gdx
...
weekdays                      Number of Weekdays in month/21/
...
Execute_Unload "Valid_April2018(2Periods).gdx";
* Dump the gdx file to an Excel workbook
Execute "gdx2xls Valid_April2018(2Periods).gdx"
